...
int main(int argc, char** argv) {
	...
	while (Solved != 1) {
		...
		if (counter != 0) {
			solve();					// Solves Sudoku as long as there are no 0s left
			counter = 0;
		} else Solved = 1;
	}
...
// Solves the Sudoku board by checking for one possibility and inputing the number
void solve() {
	int i, j;
	for (i = 0; i < 9; i++) {
		for (j = 0; j < 9; j++) {
			if (Sudoku[i][j] == 0) {
				int n, c = 0, OnePossibility = 0;
				for (n = 1; n <= 9; n++) {
